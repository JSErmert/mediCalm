# M4.0–4.5 Clarifications — v1.1 (Current)
This is v1.1. Use this file instead of any previous clarification versions.
This version supersedes all prior clarification documents.

Claude Code must use this version as the authoritative clarification layer.

Key additions in v1.1:
- Explicit HARI-compatible safety gate defined (pre-M4.3)
- M4.2 MVP intake confirmed as full replacement for PainInputScreen
- Validation gate + deletion horizon behavior fully locked
- Implementation Plan v2 alignment
...
## C4 — HARI Safety Gate (First-Pass Requirement)

**Decision:** Implement a lightweight HARI-compatible safety gate that runs AFTER M4.2 MVP intake and BEFORE M4.3 state estimation.

Purpose:
- preserve M3 safety behavior despite reduced intake detail
- prevent unsafe sessions from entering HARI flow

Flow:
- Intake (M4.2 MVP)
- Safety Gate
- If CLEAR → continue to M4.3
- If HOLD or STOP → do not proceed to session
...
- new or worsening weakness
- coordination change
- strong numbness or major sensation change
- symptoms feel severe or different enough to feel unsafe
- not sure
...
It is a **pre-engine eligibility gate**

---

### First-pass constraint

This is a minimal safety layer.
It does not fully replicate previous M3 safety depth, but preserves a safe stop path.
